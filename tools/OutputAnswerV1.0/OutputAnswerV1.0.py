...
if __name__ == "__main__":
    ...
    with open(config["relative_path"] + "/prompt.txt", "r", encoding="utf8") as f:
        prompt = f.read()
    ...
    # 发送给大模型
    output, _ = call_llm(config, messages, model=config["models"]["doubao-1-5-pro-256k"])

    # 也需要展示出来的json
    send_data_via_redis(config["redis_conn"], config["chat_id"], [
        {"type": "inline_json",
         "content": {"text": output}},
        {"type": "display_json", "content": {"text": output}},
